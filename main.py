...
def mix():
    ...
    addFileButton = Button(tk, text="+ Add a file",bg="#090979", fg="ffffff", command=addFile)
    addFileButton.pack()
    doneButton = Button(tk, text="Done", bg="#090979",
...
def color():
    ...
    try:
        ...
        tk.protocol("WM_DELETE_WINDOW", onClose)
        root.withdraw()
        scale = Scale(tk, variable=colorvalue, orient=HORIZONTAL, bg = "#090979", fg ="ffffff", from_=0, to =1, resolution=0.05)
        scale.pack()
        button = Button(tk, text="Done", bg = "#090979", fg ="ffffff",command=createVideo)
        button.pack()
        tk.mainloop()
...
root.config(bg="#090979")

mixBut = Button(root, text="Mix", relief=GROOVE,
                bg="#090979", fg="white", command=mix)
mixBut.pack(side="left", padx=20)
mixBut.config(width=8, height=3)

mirrorBut = Button(root, text="Mirror", relief=GROOVE,
                   bg="#090979", fg="white", command=mirror)
mirrorBut.pack(side="left", padx=20)
mirrorBut.config(width=8, height=3)

resizeBut = Button(root, text="Resize", relief = GROOVE, bg="#090979", fg="white", command=resize)
resizeBut.pack(side="left",padx = 20)
resizeBut.config(width=8, height=3)

speedBut = Button(root, text="Speed", relief = GROOVE, bg="#090979", fg="white", command=speed)
speedBut.pack(side="left",padx = 20)
speedBut.config(width=8, height=3)

colorBut = Button(root, text="Color", relief=GROOVE,bg="#090979", fg="white", command=color)
colorBut.pack(side="left", padx=20)
colorBut.config(width=8, height=3)

trimBut = Button(root, text="Trim", relief = GROOVE, bg="#090979", fg="white", command=trim)
trimBut.pack(side="left",padx = 20)
trimBut.config(width=8, height=3)

audioBut = Button(root, text="Audio", relief = GROOVE, bg="#090979", fg="white", command=audio)
